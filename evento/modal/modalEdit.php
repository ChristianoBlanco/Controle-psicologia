		  ...
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="evento/action/eventoEdit.php" onsubmit="return validaForm(this);">
			  ...
			  <div class="modal-body">
				  ...
				  <div class="form-group">
					<label for="cor" class="col-sm-2 control-label"><?= utf8_encode("Recorr�ncia"); ?></label>
					<div class="col-sm-10">
					  <select name="rec" class="form-control" id="rec">
							<?php 
								$sql = "SELECT * FROM eventos Where id_evento = $id_evento";
										...
										foreach($result as $selrec){
											$sel_rec = $selrec['recorrencia'];
												echo "<option value='' >{$sel_rec}</option>";
										}
											
							?>
						   <option  value="Nunca">Nunca</option>
						   <option  value="Todos os Dias">Todos os Dias</option>
						   <option  value="A Cada Semanas">A Cada Semanas</option>						  
						   <option  value="A Cada 2 Semanas">A Cada 2 Semanas</option>
						   <option  value="A Cada M�s"><?= utf8_encode("A Cada M�s"); ?> </option>
						   <option  value="A Cada Ano">A Cada Ano</option>						  
						</select>
					</div>
				  </div>
